...
![alt text](Sprint1_Task2.drawio.png)

# Описание Архитектуры Микросервисов

## 1. Обзор Декомпозированной Архитектуры
Монолитная система была разделена на отдельные микросервисы, каждый из которых отвечает за определённую бизнес-доменную область.

### Основные решения при декомпозиции:
...
- Планируется использовать хореографическую модель отката транзакций для обеспечения согласованности распределенных операций в случае отказов.
...
1. **Инициация платежа** — через API Gateway отправляется запрос в TransactionService с ID пользователя, ID заказа и суммой.
2. **Предоставление статуса платежа** — TransactionService передаёт через API Gateway данные во внешний платежный сервис.
3. **Подтверждение платежа** — через API Gateway отправляется подтверждение операции в TransactionService.
4. **Запрос статуса платежа** — API Gateway запрашивает у TransactionService текущий статус обработки транзакции.
5. **Подтверждение отмены платежа** — TransactionService уведомляет через API Gateway об успешной отмене платежа.
